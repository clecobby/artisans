...
class CognitoIdentityProviderWrapper:
    ...
    def load_credentials(self, access_key, secret_key):
        # Load credentials when needed
        self.cognito_idp_client = boto3.client("cognito-idp", region_name="us-east-1", aws_access_key_id=access_key, aws_secret_access_key=secret_key)
    ...
    def sign_up_user(self, user_name, password, user_email,phone_number):
        """
        ...
        try:
            kwargs = {
                ...
                "Username": user_name,
                "Password": password,
                "UserAttributes": [{"Name": "email", "Value": user_email},{"Name":"phone_number","Value":phone_number}],
            }
            if self.client_secret is not None:
                kwargs["SecretHash"] = self._secret_hash(user_name)
            response = self.cognito_idp_client.sign_up(**kwargs)
            logger.info('response is *** '+json.dumps((str(response))))
            confirmed = response["ResponseMetadata"]["HTTPStatusCode"]
        except ClientError as err:
            logger.info('response is *** '+str(err.response))
    ...
    def callboto_withparameterstore_accessinfo(self,user_name):
        ...
        # Create a Cognito Identity Provider client using the session
        cognito_idp_client = session.client("cognito-idp")

        # Now you can use the cognito_idp_client to make calls to Cognito Identity Provider API
